...
const notes: Record<string, Post> = {
...
export default function PostDetailPage() {
  ...
  const [post, setPost] = useState<Post | null>(notes[params.id] || null);
  const [notFound, setNotFound] = useState(false);
  const [liked, setLiked] = useState(false);
  const [comment, setComment] = useState('');
  const [comments, setComments] = useState<string[]>(['Beautifully put — congratulations to the whole team.']);
  useEffect(() => { fetch(apiUrl(`/api/posts/${params.id}`)).then(response => response.ok ? response.json() : Promise.reject()).then(data => setPost(data.post)).catch(() => setNotFound(true)); }, [params.id]);
  ...
  if (!post && notFound) return <div style={{ minHeight: '100vh', padding: '9rem 1.5rem', textAlign: 'center' }}><p className="font-display" style={{ fontSize: '2.5rem', color: 'var(--white)' }}>Post not found.</p><Link href="/posts" style={{ color: 'var(--gold)' }}>Back to posts</Link></div>;
  ...
  return <div style={{ minHeight: '100vh', maxWidth: '52rem', margin: '0 auto', padding: '9rem 1.5rem 7rem' }}>
    ...
    <article style={{ marginTop: '2rem', borderTop: '1px solid var(--border)', borderBottom: '1px solid var(--border)', padding: '2rem 0' }}>
      ...
      {post.video && <video controls style={{ width: '100%', maxHeight: '38rem', marginTop: '2rem', background: 'var(--bg)' }}><source src={post.video} type="video/mp4" /></video>}
